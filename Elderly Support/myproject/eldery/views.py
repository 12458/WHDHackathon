...
from myproject.eldery.dialogflow_gracie import Session_Client 
...
def groceries(): #DO NOT CHANGE THIS FUNCTION NAME WITHOUT CHANGING URL ABOVE
    if "user" in session:
        if request.method == "POST":
            ...
            grocer = Grocery(order,Elderlies_id)
...
@elderly_blueprints.route('/C3',methods=['GET','POST'])
def order_checkout():
    groceies_ordered = session["ABC"]
    return render_template("C3.html",elderly_order=groceies_ordered)
